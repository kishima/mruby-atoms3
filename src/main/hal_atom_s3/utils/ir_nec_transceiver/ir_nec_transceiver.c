...
//                rmt_nec_symbols[i].duration0,
//                rmt_nec_symbols[i].level1,
...
// static bool example_rmt_rx_done_callback(rmt_channel_handle_t channel, const rmt_rx_done_event_data_t* edata, void*
// user_data)
// {
...
void ir_nec_transceiver_init(int txPin)
{
    ...
    //     .signal_range_min_ns =
    //         1250, // the shortest duration for NEC signal is 560us, 1250ns < 560us, valid signal won't be treated as noise
    //     .signal_range_max_ns =
    //         12000000, // the longest duration for NEC signal is 9000us, 12000000ns > 9000us, the receive won't stop early
    // };
    ...
    rmt_tx_channel_config_t tx_channel_cfg = {
        ...
        .resolution_hz = EXAMPLE_IR_RESOLUTION_HZ,
        .mem_block_symbols = 64, // amount of RMT symbols that the channel can store at a time
        .trans_queue_depth = 4,  // number of transactions that allowed to pending in the background, this example won't queue
                                 // multiple transactions, so queue depth > 1 is sufficient
        .gpio_num = (gpio_num_t)txPin,
    };
    // rmt_channel_handle_t tx_channel = NULL;
    ESP_ERROR_CHECK(rmt_new_tx_channel(&tx_channel_cfg, &_tx_channel));
    ...
    ESP_ERROR_CHECK(rmt_apply_carrier(_tx_channel, &carrier_cfg));
    ...
    ESP_ERROR_CHECK(rmt_new_ir_nec_encoder(&nec_encoder_cfg, &_nec_encoder));
...
void ir_nec_transceiver_send(uint16_t addr, uint16_t cmmd)
{
    ...
    ESP_ERROR_CHECK(rmt_transmit(_tx_channel, _nec_encoder, &scan_code, sizeof(scan_code), &_transmit_config));
}
